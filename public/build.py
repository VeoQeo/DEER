...
"""
    ...
    def prepare_git_public(self) -> None:
        public_dir = 'public'
        self.config.log(f"Подготовка публичной директории: {public_dir}/", LogLevel.INFO)
        ...
        self.ensure_dir(public_dir)

        for file in files_to_copy:
            if os.path.exists(file):
                shutil.copy2(file, public_dir)
                self.config.log(f"Скопирован: {file} → {public_dir}/", LogLevel.DEBUG)
            else:
                self.config.log(f"Файл не найден, пропущен: {file}", LogLevel.WARNING)

        src_dest = os.path.join(public_dir, 'src')
        if os.path.exists('src'):
            if os.path.exists(src_dest):
                shutil.rmtree(src_dest)
            shutil.copytree('src', src_dest)
            self.config.log("Скопирована папка src/", LogLevel.DEBUG)
        else:
            self.config.log("Папка src/ не найдена!", LogLevel.ERROR)
            sys.exit(1)

        self.config.log(f"Публичная директория готова: {public_dir}/", LogLevel.INFO)
        
        print(f"\n{Color.GREEN}Готово! Перейдите в папку:{Color.END}")
        print(f"{Color.CYAN}cd {os.path.abspath(public_dir)}{Color.END}\n")
